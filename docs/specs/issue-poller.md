...
Job Queueへの直接投入である点がMR Pollerと異なる**(ADR-0025
...
- 非対象:
  ...
  - 一度`(project, issue_iid)`が起票されると、以後は永続的にスキップされる。MRの
    ...
    相当する「版」の概念が無いため。ADR-0025「決定」参照)
...
| `JobError` | `job_repo.enqueue` | そのIssueの処理を打ち切り、`IssuePollError(issue_iid=...)`として記録して継続(ログ`issue_poller.job_enqueue_failed`)。**Issue Ticket Storeへの記録は既に成功しているため、そのIssueは次回以降のポーリングで再試行されない**(ADR-0025「決定」の意図的なトレードオフ) |
