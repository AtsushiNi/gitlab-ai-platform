...
- ステータス: 実装済み(現時点のGitLabAdapter 9メソッド分。M2-10 [#47](https://github.com/AtsushiNi/gitlab-ai-platform/issues/47)
  マージ後に追加対応が必要。下記「対象ツール」節参照)

## 責務

対話型Claude Code(Windows VS Code拡張・CLI)が、実行中にエージェント自身の判断で
`GitLabAdapter`の許可された操作(read全般 + branch作成/push/MR作成/コメント)を能動的に
呼び出せるようにする。`GitLabAdapter`のインスタンスをMCPサーバー(stdio)としてラップし、
...
- 非対象:
  - 新しい権限の追加。`GitLabWriter`の許可リスト(ADR-0002: `create_branch` /
    `push_file_changes` / `create_merge_request` / `create_merge_request_comment`)を
    回避・拡張する経路にはならない。merge・protected branchへの直push・branch削除・
    管理操作は`GitLabAdapter`自体にメソッドが存在しないため、このブリッジ経由でも
    呼び出し不可能(`tests/gitlab_ai_platform/mcp_bridge/test_server.py`で担保)
  - 実MCPプロトコル(stdio上のJSON-RPC)のトランスポート実装そのもの。これは`mcp`パッケージ
    (MCP Python SDK)に委譲し、本コンポーネントはツールの登録・委譲のみを行う
  - GitLab以外の外部システムとの連携
  - M2-10([#47](https://github.com/AtsushiNi/gitlab-ai-platform/issues/47))で追加予定の
    `list_issues`/`get_issue`/`create_issue`/`update_issue`/`update_merge_request`は、
    本ドキュメント作成時点ではまだ`GitLabAdapter`に存在しないため対象外
    (下記「対象ツール」節のTODO参照)
...
現時点で`GitLabAdapter`に存在する9メソッドすべてを1:1でツール化している。引数・戻り値は
プリミティブ型/`dict`/`list`のみとし、`gitlab_adapter/types.py`のdataclassは
...
**TODO(M2-10 [#47](https://github.com/AtsushiNi/gitlab-ai-platform/issues/47)マージ後)**:
`list_issues`/`get_issue`/`create_issue`/`update_issue`/`update_merge_request`が
`GitLabAdapter`に追加された時点で、この表に対応する行を追加し、`tools.py`の
`TOOL_FACTORIES`にも対応するファクトリ関数を追加すること([ADR-0010](../adr/0010-gitlab-mcp-tool-bridge.md)
「決定」節のTODO参照)。
...
- `test_server.py`:
  - `ALLOWED_TOOL_NAMES`が現時点の9メソッドという決め打ちの集合と完全一致することを検証する
    (`test_allowed_tool_names_matches_current_nine_method_allow_list`)
  - `ALLOWED_TOOL_NAMES`が`GitLabReader`/`GitLabWriter`(Protocol)の公開メソッド集合と
    ...
    M2-10(#47)でAdapter側のメソッドが増えると、このテストが先に落ちて追従漏れに気づける
