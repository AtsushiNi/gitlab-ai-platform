...
- PRの本文には `Closes #<issue番号>` (または `Fixes #<issue番号>`) を含める。
  単に本文中でIssue番号に触れるだけ(例: `(#11)` とタイトルに書くだけ)では
  GitHubがマージ時に自動クローズしてくれず、Issueがマージ後もOpenのまま
  取り残される事故が複数回起きた
...
- ソースコードのコメントは日本語で書く。長文の説明は不要だが、わかりにくい処理や、
  意図がパッと読み取れない変数名・メソッドには短い説明コメントを添える
