...
`GitLabAdapter`が持つ許可されたメソッド集合(現時点で9個)以外のツールが公開されない
ことを機構として担保する。将来`GitLabReader`/`GitLabWriter`にメソッドが増えた場合
(M2-10, #47)、このテストの期待集合は意図的に更新しない限り失敗する
...
# 現時点(#62実装時点)で許可されているべきツール名の完全な集合。
# M2-10(#47)でGitLabReader/GitLabWriterにメソッドが追加されたら、この集合と
...
# GitLab Adapter側(ADR-0002)で機構的に存在しないと保証されている禁止操作。
...
def test_allowed_tool_names_matches_current_nine_method_allow_list() -> None:
...
def test_allowed_tool_names_matches_gitlab_adapter_protocol_exactly() -> None:
    # 現時点ではGitLabReader(5) + GitLabWriter(4)のちょうど9個がAdapterの全許可メソッドであり、
    # ブリッジのツール集合と完全一致するはず。#47でAdapter側のメソッドが増えると
